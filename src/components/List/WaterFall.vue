<template>
  <div class="waterfall-container" v-loading="state.loading" element-loading-text="拼命加载中..." element-loading-background="rgba(255, 255, 255, 0.8)">
...
const computedImagePos = (list: IImageItem[], isFirst: boolean) => {
  list.forEach((item, index) => {
    ...
    if (isFirst && index < props.column) {
      ...
      imagePos.value.push({
        x: minIndex % props.column !== 0 ? minIndex * state.imageWidth + props.gap * minIndex : minIndex * state.imageWidth,
        y: minHeight,
      });
